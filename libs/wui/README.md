...
1. **Atomic tokens** (`libs/design-token/src/tokens/atomic/`): colors, global font settings, **text/title font sizes** (`font.json`), spacing.
2. **Semantic tokens** (`libs/design-token/src/tokens/semantic/`): aliases (neutral, success, danger, warning, info) that reference atomic tokens.
3. **Dark theme** (`libs/design-token/src/tokens/semantic/color.dark.json`): same aliases with inverted neutral for dark mode.
...
- token enums under `libs/design-token/src/enum/` (consumed via `@wawawoom/design-token/enum` and re-exported from `@wawawoom/wui`), including `WuiTextSize` from `text.font.size` in `font.json`.

In development, `pnpm run dev` starts `watch:tokens` on **`@wawawoom/design-token`**: `nodemon` watches the `src/tokens/` folder **inside that package** (avoids nodemon ignoring paths outside `libs/wui`) and runs `build:tokens` plus `wui`’s `generate-dark-prefers.js` when JSON changes.

### Color enums (shared with RN)

`WuiColorName`, `WuiColorValue`, and `WuiColorAlias` are generated by Style Dictionary into **`libs/design-token/src/enum/`**. `WuiTextSize` is generated by the same token build. Import from **`@wawawoom/design-token/enum`** in this repo, or from **`@wawawoom/wui`** in apps (re-exported).
