...
$user = getUser();
if ($user === NULL) {
	header('Location: ./login.php?redirect='.urlencode($QRCODE_HAUT_URL));
	exit();
}

//TODO
// CHeck the cookie, and the qr code scan was made with the same phone
...
<html lang="en" class="invert">
    <head>
        ...
        <title>QR Code Haut - Cauchemar</title>
    </head>
    <body>
        <div id="main">
            ...
            <div id="rightCol">
                ...
                <div id="content" class="smallPadding">
                    ...
                    <h1>Bravo.</h1>
                    <p>
						Vous venez de scanner le QR Code "HAUT". Je dois ensuite m'assurer que vous êtes bien sur place... 
                    ...
                    <div class="alert alert-danger" id="too-far" style="display:none;" role="alert">
						Vous êtes trop loin, rapprochez vous du Waypoint "HAUT"
					</div>
                </div>
            </div>
        </div>
        ...
        <script>
			...
			var qrCodePosPHP = '<?php echo $QR_HAUT_COORDS_DEC; ?>'.split(',');
				...
				var geoSuccess = function(position) {
					...
					if (position && position.coords) {
						...
						if (positionIsInArea(
							...
							alert('Ok, vous êtes dans la zone. Toutes mes félicitations !')
							window.location = window.location.href.replace('.php', '-79da7fa7.php');
						} else {
							$('#too-far').show();
						}
					}
