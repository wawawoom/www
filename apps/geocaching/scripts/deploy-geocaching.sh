...
# Monorepo root (two levels up from apps/geocaching/scripts)
...
SOURCE_DIR="${SOURCE_DIR:-apps/geocaching}"
...
case "$COMMAND" in
  upload)
    echo "📤 Upload GEOCACHING (local → serveur)..."
...
EOF
    ;;
  download)
    echo "📥 Download GEOCACHING (serveur → local)..."
...
EOF
    ;;
  sync)
    echo "🔄 Sync GEOCACHING (local → serveur, miroir exact)..."
    echo "   Source: $SOURCE_DIR/"
...
echo ""
if [ $result -eq 0 ]; then
    echo "✅ GEOCACHING $COMMAND réussi."
    exit 0
else
