...
SOURCE_DIR="${SOURCE_DIR:-apps/kalei/src}"
FTP_DIR="${FTP_DIR:-/www/next/projects/kalei/}"

echo "🚀 Starting FTP upload (kalei)..."
...
echo ""
if [ $result -eq 0 ]; then
  echo "✅ kalei upload successful."
  exit 0
else
  echo "❌ Error during kalei upload (code: $result)"
  exit 1
fi
